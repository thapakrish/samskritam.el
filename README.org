...
Loading =samskritam.el= does not enable the global minor mode by itself.
Call =(samskritam-mode 1)= explicitly, as above, to install the default
keybindings.
...
You can hide the mode-line indicator by customizing `samskritam-mode-line`:
#+begin_src emacs-lisp
(setq samskritam-mode-line nil)
#+end_src
